export default async function handler(req, res) {
  ...
  try {
    const r = await fetch(
      `${SUPABASE_URL}/rest/v1/friend_answers?session_id=eq.${sessionId}&select=answers`,
      {
        headers: {
          'apikey': SUPABASE_KEY,
    ...
    const friendAnswers = rows.map(row => row.answers);

    if (friendAnswers.length === 0) {
      return res.status(200).json({ friendCount: 0, analysis: null });
    }
...
Напиши анализ в формате JSON (только JSON, без лишнего текста):
{
  ...
  "summary": "общее резюме 2-3 предложения"
    ...
    res.status(200).json({ friendCount: friendAnswers.length, analysis });
  } catch (e) {
